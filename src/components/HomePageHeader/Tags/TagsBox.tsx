...
const TagsBox = () => {
    ...
    const { ref: refContainer } = useOutsideClick({fn: onClickHideContainerHandle})
    ...
    return(
        <>
            <div className="max-w-half bg-black overflow-hidden">
                <ul className="flex text-sm">
                    {tags.map(el=><li key={el.name}><a href="" className="bg-black block mx-1 text-white px-5 py-1 text-sm capitalize rounded-[4px] border-[1px] border-gray">{el.slug}</a></li>)}
                </ul>
            </div>
            <div ref={refContainer} className="ml-0 mr-auto">
                <button onClick={onClickShowContainerHandle} >more</button>
                <AnimatePresence>
                    {renderContainer && <TagsContainer onClick={onClickHideContainerHandle} />}
                </AnimatePresence>
            </div>
        </>
